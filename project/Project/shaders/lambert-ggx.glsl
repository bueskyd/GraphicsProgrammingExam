...
// Small value added to some results of ClampedDot to fix an artifact
const float minClampedDot = 0.0001f;
...
float GeometrySmith(vec3 normal, vec3 inDir, vec3 outDir, float roughness)
{
	...
	float ggxIn = GeometrySchlickGGX(ClampedDot(normal, inDir) + minClampedDot, roughness);

	// Occlusion in output direction (masking)
	float ggxOut = GeometrySchlickGGX(ClampedDot(normal, outDir) + minClampedDot, roughness);
...
vec3 ComputeSpecularLighting(SurfaceData data, vec3 lightDir, vec3 viewDir)
{
	...
	float cosI = ClampedDot(data.normal, lightDir) + minClampedDot;
	float cosO = ClampedDot(data.normal, viewDir) + minClampedDot;
