...
void Material::SetShadowShader(
    std::shared_ptr<ShaderProgram> shadowShaderProgram, ShadowShaderSetupFunction setupFunction)
{
    m_shadowShaderProgram = shadowShaderProgram;
    m_shadowShaderSetupFunction = setupFunction;
}

void Material::UseShadowShader(const Light& light, const glm::mat4& lightSpaceMatrix, const glm::mat4& worldMatrix) const
{
    m_shadowShaderProgram->Use();
    m_shadowShaderSetupFunction(*m_shadowShaderProgram, light, lightSpaceMatrix, worldMatrix);
}

bool Material::CastsShadows() const
{
    return m_shadowShaderSetupFunction != nullptr;
}
