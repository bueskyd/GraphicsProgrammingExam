...
public:
    ...
    template<typename TIterator>
    unsigned int AddVertexArray(
        unsigned int vboIndex, TIterator& it, const TIterator itEnd,
        unsigned int instanceVboIndex, TIterator& instanceIt, const TIterator instanceItEnd,
        const SemanticMap& locations = SemanticMap());
    ...
    unsigned int AddSubmesh(unsigned int vaoIndex, Drawcall::Primitive primitive, GLint first, GLsizei count, Data::Type eboType, InstancingParam instancing);
    ...
    template<typename TIterator>
    unsigned int AddSubmesh(Drawcall::Primitive primitive, int firstElement, int elementCount, Data::Type elementType,
        unsigned int vboIndex, unsigned int eboIndex, unsigned int instanceVboIndex,
        TIterator it, const TIterator itEnd, TIterator instanceIt, const TIterator instanceItEnd, InstancingParam instancing, const SemanticMap& locations = SemanticMap());
    ...
    template<typename TVertex, typename TElement, typename TIterator, typename TInstance>
    unsigned int AddSubmesh(
        Drawcall::Primitive primitive,
        std::span<const TVertex> vertices,
        std::span<const TElement> elements,
        std::span<const TInstance> instances,
        TIterator it, const TIterator itEnd,
        TIterator instanceIt, const TIterator instanceItEnd,
        const SemanticMap& locations = SemanticMap());
    ...
    void SetSubmeshInstanceCount(int submeshIndex, GLuint instanceCount);
...
private:
    ...
    void SetupVertexAttribute(VertexArrayObject& vao, const VertexAttribute::Layout& attributeLayout, GLuint& location, const SemanticMap& locations, bool instanced = false);
...
unsigned int Mesh::AddVertexArray(
    unsigned int vboIndex, TIterator& it, const TIterator itEnd,
    unsigned int instanceVboIndex, TIterator& instanceIt, const TIterator instanceItEnd, const SemanticMap& locations)
{
    unsigned int vaoIndex = AddVertexArray();
    ...
    const auto& instanceVbo = GetVertexBuffer(instanceVboIndex);
    instanceVbo.Bind();
    while (instanceIt != instanceItEnd)
    {
        SetupVertexAttribute(vao, *instanceIt, location, locations, true);
        instanceIt++;
    }
...
unsigned int Mesh::AddSubmesh(Drawcall::Primitive primitive, int firstElement, int elementCount, Data::Type elementType,
    unsigned int vboIndex, unsigned int eboIndex, unsigned int instanceVboIndex,
    TIterator it, const TIterator itEnd, TIterator instanceIt, const TIterator instanceItEnd, InstancingParam instancing, const SemanticMap& locations)
{
    unsigned int vaoIndex = AddVertexArray(vboIndex, it, itEnd, instanceVboIndex, instanceIt, instanceItEnd, locations);
    ...
    return AddSubmesh(vaoIndex, primitive, firstElement, elementCount, elementType, instancing);
}
...
template<typename TVertex, typename TElement, typename TIterator, typename TInstance>
unsigned int Mesh::AddSubmesh(Drawcall::Primitive primitive,
    std::span<const TVertex> vertices, std::span<const TElement> elements, std::span<const TInstance> instances,
    TIterator it, const TIterator itEnd, TIterator instanceIt, const TIterator instanceItEnd, const SemanticMap& locations)
{
    int vboIndex = AddVertexData(vertices);
    int instanceVboIndex = AddVertexData(instances);
    int eboIndex = AddElementData(elements);
    InstancingParam instancing(true, instances.size());
    return AddSubmesh(primitive, 0, static_cast<int>(elements.size()), Data::GetType<TElement>(), vboIndex, eboIndex, instanceVboIndex, it, itEnd, instanceIt, instanceItEnd, instancing, locations);
}
